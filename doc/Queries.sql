...
SELECT P.file FROM Photos P INNER JOIN group_has_photos INNER JOIN Group G WHERE G.name = 'test';
#Query 3 (Find the names of all people from are in the "young adults" group)
SELECT P.name FROM Person P INNER JOIN group_has_members INNER JOIN Group G WHERE group_name = 'young adults';
#Query 4 (Find the names of all people who need a ride to the event named "test_name")
SELECT P.name FROM Person P INNER JOIN person_needs_ride_for_event INNER JOIN Event E WHERE E.name = 'test_name';
...
SELECT E.name FROM Events E INNER JOIN event_out_of_church INNER JOIN Address A WHERE A.street = 'street' AND A.state = 'state' AND A.city = 'city';
#Query 7 (Find the name and person ID of all people who are going to "Happy Hour" who are not in the "Adult" group)
SELECT P.name, P.pid FROM Person P INNER JOIN person_goes_to_event INNER JOIN Event E WHERE E.name = 'Adult Happy Hour' AND P.pid NOT IN (SELECT P.pid FROM Person P INNER JOIN GROUP G WHERE G.name = 'Adult');
...
SELECT eventName FROM Event E INNER JOIN person_goes_to_event INNER JOIN Person P WHERE P.name = 'Pastor Roth';
...
SELECT E.name FROM Event E INNER JOIN person_goes_to_event pge GROUP BY pge.eid ORDER BY count(pge.pid) DESC;
#Query 18 (Find the average number of pictures taken at all events)
SELECT E.name, avg(Ph.pid) FROM event_has_photos Ph INNER JOIN Photos P INNER JOIN Event E GROUP BY (E.name); 
#Query 19 find the event that the most people attended
SELECT E.name, count(pge.pid) FROM Event E INNER JOIN person_goes_to_event pge GROUP BY (pge.eid) ORDER BY (count(pge.pid) DESC)
#Query 20 (Find the names of everyone who has driven to AT LEAST one event)
SELECT DISTINCT P.name FROM Person P INNER JOIN person_drives_for_event pde;